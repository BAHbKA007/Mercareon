...
while True:
    try:
        ...
        # 000 Gemüsering
        cursor.execute(f"""SELECT DISTINCT HAN.HANVORNR, HAN.HANADRNR, Kunde.KUNKUBEZ, HAN.HANKENNG, CONVERT(DATE, HAN.HANVORDT) AS Datum
        FROM [db_GRS_000].[dbo].[tbl_HAN] AS HAN
        JOIN db_GRS_000.dbo.tbl_KUN AS Kunde ON Kunde.KUNNR = HAN.HANADRNR
        WHERE HANVORDT BETWEEN '{gestern}T00:00:00' AND '{morgen}T00:00:00'
        AND HAN.HANVORAR = '10'
        ...
        request = requests.post('https://mercareon.gemuesering24.de/qweqwe', json = json.dumps(final_data))
